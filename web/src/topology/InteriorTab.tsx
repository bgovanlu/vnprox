    ...
    <div className="space-y-4 text-xs">
      <div className="flex items-start gap-2 rounded border border-border p-2">
        ...
        <label htmlFor={`interior-toggle-${entityRef}`} className="flex-1">
          <span className="block font-medium text-fg-body">
      ...
      {!enabled && (
        <p className="text-fg-muted">Enable the toggle above to read this guest's interfaces, routes, DNS, and listening sockets.</p>
      )}

      {enabled && interiorQuery.isLoading && <p className="text-fg-muted">Reading guest interior…</p>}
            ...
            <div className="space-y-4" data-testid="interior-view">
              ...
              <section>
                ...
                <ul className="space-y-1">
                  {data.interfaces.map((iface) => (
                    <li key={iface.name} className="flex items-center justify-between rounded border border-border px-2 py-1">
                      <span className="font-mono text-fg-body">{iface.name}</span>
                      <span className="text-fg-muted">
                        {iface.mac ?? "—"} {iface.mtu ? `mtu ${String(iface.mtu)}` : ""} {iface.up ? "up" : "down"}
                      </span>
                    </li>
                  ))}
                  {data.interfaces.length === 0 && <li className="text-fg-muted">No interfaces reported.</li>}
                </ul>
              </section>
              ...
              <section>
                ...
                <ul className="space-y-1">
                  {data.addresses.map((addr) => (
                    <li key={`${addr.interface}-${addr.ip}`} className="flex items-center rounded border border-border px-2 py-1">
                      <span className="font-mono text-fg-body">
                        {addr.ip}
                        {addr.prefix ? `/${String(addr.prefix)}` : ""}
                      </span>
                      <span className="ml-2 text-fg-muted">on {addr.interface}</span>
                      {ipamBadge(addr, data.ipamDiff)}
                    </li>
                  ))}
                  {data.addresses.length === 0 && <li className="text-fg-muted">No addresses reported.</li>}
                </ul>
              </section>
              ...
              <section>
                ...
                <ul className="space-y-1">
                  {data.routes.map((route, i) => (
                    <li key={`${route.destination}-${String(i)}`} className="rounded border border-border px-2 py-1 font-mono text-fg-body">
                      {route.destination} {route.gateway ? `via ${route.gateway}` : ""} {route.dev ? `dev ${route.dev}` : ""}
                    </li>
                  ))}
                  {data.routes.length === 0 && <li className="text-fg-muted">No routes reported.</li>}
                </ul>
              </section>
              ...
              <section>
                ...
                {data.dns.nameservers && data.dns.nameservers.length > 0 ? (
                  ...
                  <p className="text-fg-muted">No DNS configuration reported.</p>
                )}
              </section>
              ...
              <section>
                ...
                <ul className="space-y-1">
                  {data.listeningSockets.map((sock, i) => (
                    <li key={`${sock.proto}-${sock.localAddr}-${String(sock.localPort)}-${String(i)}`} className="rounded border border-border px-2 py-1 font-mono text-fg-body">
                      {sock.proto} {sock.localAddr}:{sock.localPort}
                    </li>
                  ))}
                  {data.listeningSockets.length === 0 && <li className="text-fg-muted">No listening sockets reported.</li>}
                </ul>
              </section>
            </div>
          );
