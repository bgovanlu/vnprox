    ...
    <li className="flex items-start justify-between gap-3 border-b border-slate-100 py-1.5 text-sm last:border-0 dark:border-slate-800">
      ...
      <div className="shrink-0 text-right">
        {factor.evaluated ? (
          ...
          <span className="text-xs uppercase tracking-wide text-slate-400">Not evaluated</span>
        )}
      </div>
    </li>
  );
...
export function EmbedPosture() {
  ...
  if (isLoading) {
    return <p className="text-sm text-slate-500">Loading posture…</p>;
  }
  ...
  if (error) {
    ...
    if (error instanceof ApiError && error.status === 404) {
      return (
        <p className="text-sm text-slate-500" data-embed-state="posture-unavailable">
  ...
  return (
    <div className="flex flex-col gap-4" data-testid="embed-posture">
      <div className="flex items-baseline gap-3">
        ...
        <span className="text-sm text-slate-500">/ 100 overall</span>
        {data.qualified ? (
