...
//     before it goes stale. This is the mode vnprox's auth bridge (T-105)
//     uses to perform writes with the logged-in user's own PVE privileges.
...
// calls (cluster status/resources, node network reads/writes/reload,
// guest config reads/writes, SDN reads, firewall reads, task get/wait) —
// deliberately no generic "call any PVE endpoint" escape hatch, so every
// PVE call vnprox makes is visible at a client call site and reviewable.
//
