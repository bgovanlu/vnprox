...
// Shared test scaffolding: spin up internal/pvemock behind an httptest.Server
// and run a real internal/collect.Collector against it to populate a real
// inventory.Graph — the same pattern internal/topology's own tests use —
// reused here so this package's acceptance tests exercise the full
// pvemock -> collect -> inventory.Graph -> pbs.Project pipeline against a real
// fixture rather than hand-built graphs.
...
const fixtureBackupPaths = "../../testdata/pbs/backup-paths.yaml"
...
// build a real inventory.Snapshot, and discovers PBS Status (storages +
// backup jobs) via the same *pve.Client the collector used — the two live
// reads pbs.Project's real caller (cmd/vnproxd) combines.
func buildSnapshotAndStatus(t *testing.T, fixturePath string) (inventory.Snapshot, pbs.Status) {
	t.Helper()
