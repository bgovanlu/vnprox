...
//
// It was extracted from internal/backup (T-1902's support-bundle redactor)
// by T-2502, unchanged in behaviour, for one reason: the PVE cassette
// recorder must apply *the same* vocabulary, and internal/backup cannot be
// imported by internal/pve or internal/pvemock (backup -> host -> pvemock
// is an existing edge, so the reverse is a cycle, and the production PVE
// client has no business depending on the archive/SQLite layer). Two
// copies of the rules would be worse than one shared package: the failure
// mode this vocabulary defends against is a term one copy knows and the
// other does not — which is precisely the bug
// TestBundle_AC1_ContainsNoSecretClass caught while T-1902 was being
// written.
//
// internal/backup keeps its own exported names (backup.Redacted,
// backup.Scrub, ...) as thin aliases onto this package, so every existing
// call site and test is unaffected.
//
// Two mechanisms live here:
//
//   - **Replacement** (Scrub, JSON, OptionValue): what a support bundle
//     does. Credential-shaped material is substituted with Placeholder so
//     the surrounding diagnostic context survives.
//   - **Detection** (ScanText, ScanJSON, in scan.go): what T-2502's
//     cassette recorder does. A recorded PVE response that contains a
//     credential is not redacted and written — the write *fails*, naming
//     the field, because a fixture with a hole where a ticket used to be
//     is a fixture that no longer describes what PVE returned.
//
// Both mechanisms run off the same patterns below, so a term added for one
// is live for the other on the same commit.
//
// The rules are deliberately conservative in one direction only: they
// over-match. A redacted MTU is a mild annoyance; a WireGuard private key
// in a support bundle (or a checked-in cassette) is the failure this code
// exists to prevent.
package redact
...
// Placeholder is the marker every redactor substitutes. It is a single
...
// secret and replace it again. backup's TestScrub asserts that on every
// row.
//
// The name in the marker still says "support bundle" because that string
// is load-bearing: it appears in bundles operators have already been sent,
// and in this repo's own bundle documentation. Its value is a
// compatibility surface, not a description of every caller.
const Placeholder = "[REDACTED-BY-VNPROXCTL-SUPPORT-BUNDLE]"
...
// allowlist in backup's bundleschema.go is the control and this is
// belt-and-braces. This pattern is the *primary* control only for shapes
// vnprox does not own (a changeset op's params, an interfaces(5) file an
// operator hand-edited, a PVE API response body), where there is no finite
// field set to allowlist against.
var secretKeyPattern = regexp.MustCompile(`(?i)(` + secretTerms + `|` +
	`cred$|authorization|auth$|bearer|cookie|session_?id|` +
	`_enc$|_hash$` +
	`)`)

// SecretKey reports whether name is a key whose value must never be
...
//
// Order matters: the specific shapes run before the generic
// keyed-assignment rule so they keep their more informative replacements.
var valuePatterns = []struct {
	re *regexp.Regexp
	// name is the rule's stable identifier. It is what a cassette's
	// refusal error reports, so it is part of that error's contract.
	name string
	...
	},
	{
		...
		// a credential, and the bias is to over-redact.
		name: "pve-api-token",
...
//
// Used on every byte of every free-form thing a support bundle carries: log
// lines, changeset titles, error strings, certificate subjects, and the
// string leaves of any JSON the walk below visits. It is idempotent and
// never lengthens a line unboundedly.
...
// JSON parses raw as JSON and returns it re-encoded with every value under
// a secret-looking key replaced by Placeholder and every remaining string
// leaf passed through Scrub.
//
// This is what makes it safe to put a changeset's ops, plan, apply log and
// findings into a support bundle. Those are `json.RawMessage` columns
// holding op parameters whose shape is defined by internal/change and grows
// every phase — internal/api's redactOpSecrets strips exactly one known
// field (WgPeerAddParams' preshared key) from exactly one known op type,
// which is the right tool for an API response typed as []change.Op and the
// wrong one here: a bundle must be safe against the op type that lands next
// month. Walking the JSON by key name is type-agnostic, so a new sealed
// field called anything SecretKey matches is redacted the day it appears
...
func JSON(raw []byte) json.RawMessage {
	...
	if err := dec.Decode(&v); err != nil {
		return json.RawMessage(mustJSON(Placeholder + " (unparsable JSON)"))
	}
	out, err := json.Marshal(redactValue(v))
	if err != nil {
		return json.RawMessage(mustJSON(Placeholder + " (unencodable after redaction)"))
	}
	return out
}

// redactValue is JSON's recursive walk.
...
// OptionValue applies the key-name rule to a single key/value pair from a
// format with no JSON structure — an interfaces(5) option line, a TOML key.
// Returns the value to emit and whether it was redacted.
func OptionValue(key, value string) (string, bool) {
