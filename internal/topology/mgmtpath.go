...
// these). Exported (T-1206) so another package needing "which bond/PhysNic
// does this carrier's traffic ultimately ride" — internal/pbs's backup-path
// network sizing hint, which resolves a node's egress interface toward its
// PBS server and reports that path's bottleneck link speed — shares this one
// resolver rather than re-implementing the bond/VLAN-transitive walk a
// second time. ResolveMgmtPaths above is its original caller.
