...
func setupFindings(ctx context.Context, graph *inventory.Graph, driftSvc findings.DriftProvider, topoSvc *topology.Service, metricsSampler *metrics.Sampler, mgmtSvc findings.MgmtProvider, corosyncSvc findings.CorosyncProvider, fwAnalyticsSvc findings.FwAnalyticsProvider, scheduleSvc findings.ScheduleMissedProvider, latMeshSvc findings.LatMeshProvider, mtuSvc findings.MTUProvider, wgSvc findings.WGProvider, wanSvc findings.WanProvider, flowSvc findings.FlowProvider, k8sPoller *k8s.Poller, cephSvc findings.CephProvider, rogueSvc findings.RogueProvider, protectedSegments []string, capacitySvc findings.CapacityProvider, baselineSvc findings.BaselineProvider, federationSvc findings.FederationProvider, peerTrustSvc findings.PeerTrustProvider, storeCapacitySvc findings.StoreCapacityProvider, webhookRepo *store.WebhookRepo, notifier findings.Notifier, ws findingsBroadcaster, ipamSvc *ipam.Service, probeRepo *store.SimDivergenceRepo, thresholds findings.HealthThresholds, haSvc findings.HAReplicationProvider, logger *slog.Logger) *findings.Engine {
	return findings.New(findings.Config{
		...
		// T-1503: Ceph network-awareness seam for
		...
		// T-1601: baselineService (baseline.go) runs internal/baseline.Detect
		// over stored profiles + a recent flow window each findings cycle,
		// backing new_port/volume_spike/new_subnet (source "baseline") — nil-
		// safe, same degraded-mode convention every other producer here uses.
		Baseline: baselineSvc,
		// T-1407: federationTunnelAdapter (federationtunnel.go) resolves
		// clusters.wg_tunnel_id against the live WireGuard state, backing
		...
		// T-1905: storeCapacityAdapter (storecapacity.go) reports the app
