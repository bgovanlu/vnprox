...
**Known gap at the time of this report, since closed (T-1907):** "physical layer collapses to per-node summary" had not been implemented in any phase at the time this measurement was taken — only guest-layer collapse existed. It was not release-blocking then (the physical layer is only ~50-65 elements cluster-wide at the documented scale target, far below where collapsing would matter; guest-layer collapse — the actual pressure point, since 300 guests dominates the entity count — was and remains real and verified above), and T-1907 has since implemented it server-side (`internal/topology/collapse_physical.go`, mirroring `collapse.go`'s mechanism, threshold provisional pending T-1808's real-hardware numbers — see `docs/features/topology.md` §4 and that constant's own doc comment). At the documented scale target itself (6 NICs/node, under the 8-NIC threshold) it still never engages, so this measurement's own numbers are unaffected by its addition.
