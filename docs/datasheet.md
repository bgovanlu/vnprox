...
| Surface | Detail |
|---|---|
| **Web UI** | 26 screens, React 18 + TypeScript. Keyboard-driven (`/` search, `⌘K` palette, `g`+key navigation, `F1` help). WCAG AA, axe-gated. Dark by default. Responsive triage layout at phone width. |
...
| **REST API** | 186 routes, contract-frozen at v3.0 and additive-only since. Capability-gated from PVE ACLs. |
...
- **TLS everywhere**, reusing the node's PVE certificate by default, so there is one certificate story. Strict CSP, HSTS, `X-Frame-Options: DENY`.
...
| | |
|---|---|
| Go (production / test) | 138,136 / 112,788 lines |
| TypeScript (production / test) | 50,855 / 23,740 lines |
| Automated tests | **4,058** (2,558 Go incl. fuzz, 1,500 web) |
| Packages with tests | 68 / 73 — the five without are mock servers |
| Web feature modules with tests | 38 / 38 |
| Changeset operation types | 76 |
| Findings sources / checks | 15 / 43 |
| Schema migrations | 34, forward-only |
| Quality gate | `make check`: gofmt, vet, golangci-lint, eslint, tsc, all tests, govulncheck, npm audit |
...
| Area | Limit |
|---|---|
| **Hardware validation** | Development is mock-first against `internal/pvemock`. **6 of 123 hardware-validation items are confirmed on real Proxmox.** Multi-node behaviour — distributed apply, rollback, drift, federation, HA failover, switch push — is unproven on physical clusters |
| **End-to-end suite** | 35 Playwright specs exist but are run by no automated gate |
