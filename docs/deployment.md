...
8. Repeats 3–7 on the remaining nodes (via SSH root, same mechanism `pvecm` setups already rely on), or prints per-node instructions if SSH between nodes is unavailable. This SSH rollout re-runs `vnprox-setup` on every remaining node, including step 5 — since the token already exists cluster-wide by then, `vnprox-setup` safely detects this and warns rather than regenerating (see step 5's note above) — and `install.sh` itself then copies the first node's working `/etc/vnprox/keys/pve-token` to that node over the same root SSH channel (`scp -p` straight to the final path inside a `0700 root:root` directory, `chown root:root`/`chmod 0600` reasserted explicitly on arrival, then `systemctl restart vnprox` there) before moving on to the next node. **Fixed 2026-08-19** (debt sweep item 8) — earlier releases installed and configured every subsequent node but never copied the token, leaving them unable to authenticate to PVE until an operator did this by hand; that gap is closed as of this change. If a node couldn't be reached over SSH (or the token copy itself fails — e.g. this node's own token provisioning never produced a local file), `install.sh` falls back to printing the manual per-node recipe below (`scp` it over, `chmod 0600`, `systemctl restart vnprox`) for that node instead, rather than silently leaving it broken.
