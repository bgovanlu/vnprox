...
### 5.1 The e2e suite is enforced by nothing — `T-1806-bug-01`

35 Playwright specs exist. No `make` target and no CI job runs them. Task reports across three arcs cite passing e2e specs as evidence; none of those claims has been re-checked since the day it was written. **Impact: every ✗/◐ in the E2E column above is unverifiable.** This is the highest-leverage open item that requires no hardware.
...
|---|---|---|---|
| `T-2003-bug-01` | High | GUI | Nav-rail navigation dead-ends after the inspector closes |
...
| `T-1806-bug-01` | High | Process | See §5.1 |
| `T-1806-bug-02` | Medium | CI | See §5.2 |

### 5.6 Licensing — no `LICENSE` file exists

The repository has no license. `T-2105` plans Proxmox-community distribution and `T-2102` a public apt repository; both are blocked in practice until this is decided, and no external party can legally redistribute or contribute in the meantime. **This is not tracked by any existing card.**
