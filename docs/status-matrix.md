...
| 72 | i18n | ◐ | ◐ | — | ○ | ● | ● | ○ | — | **Framework wired, one screen localized** (`T-3106`, closes `T-2006`) — `react-i18next` v17 + `i18next` v26, bundled JSON resources (no runtime/CDN fetch). Bounded subset: `web/src/onboarding/OnboardingWalkthrough.tsx` only, string-extracted with real i18next pluralization (`health.severity.*`) and a `<Trans>` interpolation case. `web/src/onboarding/i18nCoverage.test.ts` gates new hardcoded copy in that subset; every other one of this table's ~76 other feature areas remains unlocalized — extending coverage means adding that area's own `i18n` namespace (`web/src/i18n/i18n.ts`'s doc comment), not growing the onboarding one. English is the only shipped locale: `fr` is a machine-quality stub that proves the pipeline round-trips (`web/src/onboarding/i18nLocale.test.tsx`), reachable only via `i18n.changeLanguage()` in a test — no browser-language detection, no user-facing switcher exists yet |
...
| 77 | Signed apt repository | ◐ | — | — | — | ● | ○ | — | — | **Machinery built, not hosted** (`T-2102`, `v4.0.0`: `packaging/build-apt-repo.sh`, `docs/../packaging/apt-repo.md`). `apt.vnprox.com` does not exist; `install.sh` fails closed with no signing key to verify against. Rescheduled as `T-3301` |

**Totals:** 77 feature areas · **69 complete (●) · 7 partial (◐) · 1 not started (○)** on the backend axis. (Recounted here alongside the `T-3106` row edit above — the previous 68/6/3 split predated that edit and no longer matched a mechanical count of this table even before it.)
