...
An opt-in browse/install client (`internal/hub`, `internal/api/hub.go`, `web/src/hub/`) over a public registry of §5 signed blueprint bundles and T-1702 SDK plugins. **This repo ships the client and the registry-index contract, not the registry service** — the registry (its `index.json` and hosted artifacts) lives in a separate repo, the same "provider/collection source lives elsewhere" boundary T-1106 draws for spec sources. The hub is a catalog/install-orchestration layer, **not a new trust boundary**: it inherits §5's signature + trust-store gate and T-1702's capability-scoped registry wholesale and adds no implicit-trust path.
...
**Registry index contract.** `GET <registry>/index.json` → `{schemaVersion: 1, entries: [{type: "blueprint"|"plugin", id, name, version, publisher?, description?, artifactUrl, signature?, transport?, capabilities?, extensionPoints?}]}` (`docs/api.md`'s Hub section). `GET /hub/index` proxies and briefly caches this, annotating each entry with a `vetted` flag. An entry's `artifactUrl` must resolve to the registry's own host — the fetched index is not signed as a whole, so it can never steer an install to an off-origin artifact.
