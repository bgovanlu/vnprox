...
## 7. Blueprint & plugin hub (T-1705)

An opt-in browse/install client (`internal/hub`, `internal/api/hub.go`, `web/src/hub/`) over a public registry of §5 signed blueprint bundles and T-1702 SDK plugins. **There is no registry service**: as of T-2803 this repo ships the index format, the signing/verification code (`internal/hubreg`) and the publisher tooling (`vnproxctl hub`), and the registry itself is a static directory — `index.json` next to an artifact tree — on object storage or GitHub Pages, the same "nothing to operate" posture T-2102 chose for the apt repository. See `docs/hub-registry.md`. The hub is a catalog/install-orchestration layer, **not a new trust boundary**: it inherits §5's signature + trust-store gate and T-1702's capability-scoped registry wholesale and adds no implicit-trust path.

**Enablement.** Off unless `[hub] registry_url` is set. With no registry configured the hub routes are not mounted and the UI shows an unavailable state.

**Registry index contract.** `GET <registry>/index.json` → `{schemaVersion: 1, entries: [{type: "blueprint"|"plugin", id, name, version, publisher?, description?, artifactUrl, signature?, transport?, capabilities?, extensionPoints?}]}` (`docs/api.md`'s Hub section). `GET /hub/index` proxies and briefly caches this, annotating each entry with a `vetted` flag. An entry's `artifactUrl` must resolve to the registry's own host — an index can never steer an install to an off-origin artifact.

**Signed index and revocation (T-2803).** A published index carries three additive fields the pre-T-2803 client simply ignores: `generatedAt`, `revocations`, and a `signature` by the registry's own index key (T-1107's Ed25519 envelope, over the document with `signature` removed). When an operator pins that key's fingerprint in `[hub] index_signers`, the daemon puts `internal/hubreg`'s gate on the client's existing HTTP seam — `internal/hub` itself is unchanged — and: an index that is unsigned, corrupted, truncated or signed by another key yields **an error and zero entries, never a partial catalog**; a revoked artifact is not offered and its download is refused. Because revocations live inside the signed index, that refusal costs **no network access beyond the index the client already fetched**. None of this makes an artifact trusted: the artifact's own signature and the per-installation trust store remain the only install gate.

**Install (`POST /hub/install`).** A **blueprint** install downloads the bundle and runs it through §5's *exact* `POST /blueprints/import` path (the same `importBundleCore` — verify → trust decision → save + `blueprint.import` audit — reused, not duplicated), returning the identical four-outcome status. A **plugin** install downloads a `{manifest, signature}` artifact, verifies the Ed25519 signature over the canonical manifest against the same trust store (the same `blueprint.VerifySignature` primitive §5 uses), applies the identical unsigned/untrusted/invalid gate, and — only once trust permits — installs the manifest through T-1702's registry, which independently re-validates the capability scope. An unsigned or untrusted-signer artifact of either kind is never installed without the explicit `trustUnsigned`/`trustNewKey` step §5 already defines.

**Vetted tier.** `[hub] vetted_signers` is the hub's own recognized-signer fingerprint allowlist, **distinct** from §5's per-admin trust store. A "vetted" badge means only "this fingerprint is on the hub's recognized list" — it is purely informational and **never** bypasses the ordinary trust decision: a vetted-but-not-yet-trusted-here signer still requires an explicit trust step at install.

**UI.** `web/src/hub/HubPage.tsx` browses blueprints/plugins, surfaces a plugin's declared capability scope for review *before* an install is confirmed, and reuses §5's trust-status vocabulary (`unsigned`/`untrustedSignature`/`invalidSignature`) for the shared signature gate — the vetted badge is shown but never substitutes for the trust confirm.
