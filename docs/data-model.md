...
| Type | Key fields |
|---|---|
| `PhysNic` | name, mac, driver, speedMbps, duplex, mediaPort (T-3503: `""`\|`tp`\|`aui`\|`mii`\|`fibre`\|`bnc`\|`da`\|`none`\|`other`, lowercased from linux/ethtool.h's PORT_* constants read off the same SIOCETHTOOL/ETHTOOL_GSET ioctl as speed/duplex — host-netlink-only, reported even when the link has no carrier, unlike speedMbps; see `planning/reports/evidence/pve-9.2.4-nic-media-and-speed.txt`), linkUp, mtu, pciAddr, sriovVFs []VirtualFunction (T-1506, given full shape below — host-netlink-only, excluded from the merge/provenance/delta machinery exactly like Bridge.fdb below, since it is one contributing source's live hardware/driver state, not a config value multiple sources could disagree on), pending |
...
| `Bond` | name, mode (802.3ad, active-backup, ...), slaves []string, lacpRate, xmitHashPolicy, miiStatus, activeSlave, mtu, pending, slaveDetail []BondSlave (per-slave runtime status, host-netlink-only), ovsBridge (OVS bonds only — the `ovs_bridge` attachment; without it an OVS bond is not re-creatable, see T-3105) |
...
| `Bridge` | name, kind (linux|ovs), ports []Ref, vlanAware bool, vids []VidRange, stp, mtu, addresses []CIDR, gateway, comments, pending, fdb []FDBEntry (T-306, added retroactively per docs/development.md's definition-of-done #4: `{mac, port, vlan, master bool, permanent bool, stale bool}`, host-netlink-only — excluded from the merge/provenance/delta machinery every other field here goes through, since FDB churns on every poll and has only one contributing source; see `internal/topology.FDB`/`FDBSearch` for the cluster-wide, ownership-labeled view built from it), stpState *BridgeSTP (T-3901, host-netlink-only like fdb above — nil when no live sysfs STP read succeeded for this bridge: `{rootId, bridgeId, stpState int, priority, rootPort, rootPathCost, isRoot bool, ports []BridgePortSTP}` where `BridgePortSTP` is `{port, designatedRoot, designatedBridge, state, role, portNo, pathCost, priority, designatedCost}`. `state` is the kernel's own BR_STATE_* vocabulary (`disabled\|listening\|learning\|forwarding\|blocking`, confirmed against `bridge -d link show`'s identical strings); `role` (`root\|designated\|blocking\|disabled`) is *derived*, not read — the Linux kernel's built-in bridge STP is classic 802.1D and exposes no native role attribute (`internal/host/stp.go`'s `deriveBridgePortRole`). `isRoot` (`rootId == bridgeId`) is misleadingly true for *every* bridge when `stpState` is 0 (STP administratively off — pvecube's actual observed default on every bridge): with no protocol running there is no root election, so the kernel just reports each bridge as its own root. Every consumer (the map's `stp-root`/`stp-role=`/`stp-state=` badges, `internal/topology/project.go`'s `badgesOf`/`stpPortBadges`) gates on `stpState != 0` before treating `isRoot`/`role` as meaningful. See `planning/reports/evidence/pve-9.2.4-bridge-stp-2026-08-27.txt` for the live field values this was modeled from. |
...
| `SdnController` (T-3102) | id (`[a-zA-Z][a-zA-Z0-9_-]*[a-zA-Z0-9]`, real PVE 9.2 pattern, `pve-9.2.4-sdn-schema.txt`), type (bgp\|evpn\|faucet\|isis — real PVE 9.2 enum, same capture), node, nodes []string, loopback (general — every type), plus type-conditional fields: asn/bgpMode/bgpMultipathAsPathRelax/ebgp/ebgpMultihop/peers []string (bgp), fabric/peerGroupName/routeMapIn/routeMapOut (evpn), isisDomain/isisIfaces []string/isisNet (isis) — see `planning/reports/needs-hardware-validation.md`'s T-3102 entry on why this per-type field assignment is inferred from each field's description rather than read off an explicit "Conditional options:" grouping the way `SdnFabric`'s is. Unlike `SdnFabric`, a controller IS a live-polled `internal/inventory` graph entity (`KindSDNController`) — see footnote ¹ |
| `SdnIpam` (T-3104) | id (`[a-zA-Z][a-zA-Z0-9]*[a-zA-Z0-9]`, real PVE 9.2 pattern, `pve-9.2.4-sdn-schema.txt`), type (netbox\|phpipam\|pve — real PVE 9.2 enum, same capture; vnprox already modeled this correctly, no capture-vs-model correction needed here), url, fingerprint, section — plus, on the write path only (`SdnIpamCreateParams`/`SdnIpamUpdateParams`, never on a read): token. This is the configured IPAM *plugin object* itself (its connection config), not an allocation — `ipam.alloc.*`'s targets are unrelated. Like `SdnController`, an ipam instance IS a live-polled `internal/inventory` graph entity (`KindSDNIpam`), not a `SdnFabric`-style "Ref only" kind — see the paragraph below footnote ¹'s SdnController one for why |
...
| `FwRuleset` | scope (cluster\|node\|guest\|vnet — `vnet` added by T-3103: `/cluster/sdn/vnets/{vnet}/firewall/{rules,options}`, hardware-captured, `pve-9.2.4-sdn-schema.txt`), ref (vnet scope: `fw-ruleset::vnet/<zone>/<vnet>`), enabled, defaultIn/Out policy, defaultForward/logLevelForward (T-3103: the forward chain's own fallthrough policy/log level — defaultForward at cluster/node/vnet scope, ACCEPT\|DROP only, no REJECT; logLevelForward hardware-confirmed only at vnet scope), rules []FwRule, aliases []FwAlias, ipsets []FwIPSet, groups []FwGroup — vnet scope has no aliases/ipsets/groups of its own (hardware-captured, same gap node scope has, T-608) |
| `FwRule` | pos, enabled, direction (in\|out\|forward\|group — `forward` added by T-3103, hardware-confirmed at cluster/node/vnet scope, rejected at guest scope), action, proto, source, dest, sport, dport, iface, macro, log, comment |
...
`SdnController` (T-3102) deliberately does **not** follow footnote ¹'s pattern: it IS an
`internal/inventory` graph entity (`KindSDNController`), live-polled by the same `pollSDN` step as
`SdnZone`/`SdnVnet`/`SdnSubnet`/`SdnDnsZone`/`SdnDnsRecord`, not a Fabric-style "Ref only, read live
by `internal/sdn.Service` alone" kind. The reason is acceptance criterion 5's referential need: an
`sdn.controller.delete` op must be blockable when a zone's own `controller` field still names it, and
that check (`internal/change/validate_referential.go`'s `checkSdnControllerDeletable`) scans the live
inventory snapshot's `SdnZone` entities — a capability a Fabric-style "not in the graph at all" model
...
CREATE TABLE neighbor_bindings (    -- T-3905: internal/store/migrations/0050_neighbor_bindings.sql
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  at INTEGER NOT NULL, node TEXT NOT NULL,  -- node is always the LOCAL node (see prose note)
  ip TEXT NOT NULL, mac TEXT NOT NULL,
  prev_mac TEXT,                    -- NULL for this (node, ip)'s first-ever row
  iface TEXT NOT NULL DEFAULT '', state TEXT NOT NULL DEFAULT ''
);  -- APPEND-ON-CHANGE, not append-on-every-poll: a row is written only when
    -- a (node, ip)'s resolved MAC first appears or differs from the last
    -- recorded MAC for that (node, ip) — see internal/neighbor/history.go's
    -- doc comment. Bounded: pruned to a 24h retention window AND a hard row
    -- cap (50,000), whichever is smaller prunes first, the same tick-based
    -- prune-loop pattern flow_samples/latency_samples already establish.
    -- NOT a long-term warehouse.
...
**`flow_samples` (T-1002: flow ingestion engine, docs/api.md's "Flows" section, `internal/store/migrations/0007_flows.sql`).** One row per decoded `flow.Record` (sFlow v5, NetFlow v5/v9, or IPFIX — `source` names which), observed by this node's own opt-in UDP listeners (`[flows]` in `vnprox.toml`, off by default per node). T-1004's `internal/flow/hostsample` package feeds this exact same table for nodes with no external exporter (`source = "conntrack"`, also opt-in/off-by-default via `[flows] conntrack_sampling_enabled`/`ebpf_sampling_enabled`) — no second storage path. T-3706's `[flows] dev_fixture_dir` (`cmd/vnproxd/flows_fixture.go`) additionally seeds a static dev-only corpus tagged `source = "fixture"`, never emitted by a listener or sampler and never set outside a dev config — kept distinct so a seeded row is never mistaken for a real observation. Unlike `metric_samples`' `(ref, at)` natural key, there is no dedup key here — many distinct flow observations legitimately share the same `(node, src, dst, port, at)` tuple at one-second resolution, so `id` is a plain autoincrement surrogate, also used as `GET /api/peer/flows`' cluster-merge pagination tiebreak (docs/api.md's `FlowRecord.id`). `src_ref`/`dst_ref` are inventory `Ref` strings, populated only when `src_ip`/`dst_ip` resolves against a known bridge or SDN subnet in the live inventory graph (`internal/flow.GraphResolver`) — empty otherwise, never guessed. Bounded by **both** a retention window (`[flows] retention_minutes`, default 60) **and** a hard row cap (`[flows] max_rows`, default 2,000,000), whichever is smaller prunes first, on the same tick-based prune-loop pattern `metric_samples`' `RunPruneLoop` already establishes — this is explicitly **not** a long-term flow warehouse (docs/roadmap-next.md's carried-forward invariant); see `internal/flow`'s package doc comment. T-1504's `serviceClass` attribution (docs/api.md's Flows section) is deliberately **not** a column here: it is recomputed fresh from a row's own `src_ip`/`dst_ip`/`src_port`/`dst_port`/`proto`/`vlan` by `internal/flow.Classifier` at `GET /flows` read time, so a row classifies correctly against whichever `NetworkSource`s are registered *now* — including one registered after the row was ingested (e.g. T-1503 registering Ceph CIDRs later) — the same "recompute from live state" stance `internal/ipam`'s conflict findings already take, rather than freezing a stale classification into storage.

**`neighbor_bindings` (T-3905: neighbor binding timeline, docs/api.md's "Neighbor binding history" section, `internal/store/migrations/0050_neighbor_bindings.sql`).** App-owned per this doc's top-level rule — vnprox's own observation of the local node's resolved ARP/IPv6-neighbor table over time, never a shadow copy of anything PVE owns. One row per IP↔MAC binding *transition*, observed by this node's own local `internal/host.Reader.Neighbors` read (`internal/neighbor.HistoryRecorder`, polled every 15s) — **never a peer's**: `node` is always the local node, the same `flow_samples`-style node-local-storage-plus-read-time-fan-out design (docs/architecture.md §7), not `metric_samples`' documented centralized-fan-out-then-store exception. Append-**on-change**, not append-on-every-poll: a row is written only when a (node, ip)'s resolved MAC first appears or differs from the previously-recorded MAC for that (node, ip) (`HistoryRecorder.Poll` compares against `NeighborBindingRepo.LatestByIP` each tick) — a stable network's ring stays small on its own, and a burst of rows for one (node, ip) or one (node, mac) in a short window *is* the flap signal, not a derived count over a per-poll snapshot ring. `prev_mac` is `NULL` for a (node, ip)'s first-ever recorded row (a discovery, not a rebind) — `CountSince`'s flap-window query excludes these on purpose (a first sighting is never a "transition"). `id` is a plain autoincrement surrogate (like `flow_samples`, unlike `metric_samples`' natural key), also used as `GET /api/peer/host/neighbors/history`'s cluster-merge pagination tiebreak. Bounded by **both** a retention window (24h, matching `metric_samples`' documented window — a "what changed in the last day" timeline is this feature's useful shape, not `flow_samples`' shorter 60-minute window) **and** a hard row cap (50,000 — sized down from `flow_samples`' 2,000,000 for append-on-change's much lower expected row rate), whichever is smaller prunes first, the same tick-based prune-loop pattern `flow_samples`/`latency_samples` already establish — **not** a long-term warehouse. Flap detection (`neighbor_binding_flap`, `source: "health"` in `GET /findings`) is **not** a column here either: it is recomputed fresh from this table by `internal/neighbor.HistoryRecorder.Flaps` at findings-cycle time (`IPFlapThreshold`=3 within `IPFlapWindow`=2m, or `MACClaimThreshold`=5 within `MACClaimWindow`=2m), the same "recompute from live state, don't freeze a verdict into storage" stance `capacity_aggregates`' forecast findings and `flow_samples`' `serviceClass` attribution both take.
...
**`switch_snmp_targets` (T-4013: read-only SNMP switch-counter poller, docs/api.md's "Switch counters (SNMP)" section, migration `internal/store/migrations/0054_switch_snmp_targets.sql`).** App-owned intent + credentials only per this doc's top-level rule — an operator's per-switch opt-in to SNMP polling, keyed by the switch's LLDP `chassis_id` (unique-indexed); the switch's live port counters are never shadow-copied here, only polled fresh every tick (`internal/ifcounters.Service`, current state, not a ring — no history table, the same "current state, not a ring" shape `mtuprobe`'s in-memory-only results follow). Distinct from the `switches` table (T-1205's guarded-push registry, `internal/switchdrv`'s credentials for the OpenConfig/gNMI write path): a chassis can have a `switch_snmp_targets` row without ever being registered for push at all, and `internal/ifcounters` does not import `internal/switchdrv` or call any of its methods (`internal/ifcounters/noswitchdrv_test.go`'s source-scan assertion). `community_enc` is AES-256-GCM ciphertext (`internal/store/cipher.go`'s `SessionCipher` — the identical cipher/key `switches.credentials_enc` uses, not a second key pair), registered in `internal/backup/secrets.go`'s secret-class inventory; never returned by any API response (`GET /snmp/targets` reports `hasCommunity: bool` only). `enabled` defaults to 0 (false): polling a switch is per-switch explicit opt-in, off by default, though — unlike `switches.enabled`'s two-key interlock with a daemon-level `[switches] enabled` master flag (a write path, deliberately harder to turn on) — there is no second daemon-level flag here, since a low-rate outbound SNMP GET/GET-BULK toward an already LLDP-known switch carries no listening-port attack surface to gate behind one (the same reasoning `[latmesh]`/`[mtuprobe]`/`[wan]` already document for their own always-on schedulers).
...
| sdn | `sdn.zone.create/update/delete`, `sdn.vnet.create/update/delete`, `sdn.subnet.create/update/delete`, `sdn.fabric.create/update/delete` (T-3101 — an SDN Fabric, PVE 9's underlay-routing object family; stages/applies through this same group's `sdn.apply`, no bespoke path), `sdn.controller.create/update/delete` (T-3102 — an SDN Controller, the BGP/EVPN/faucet/isis underlay-control-plane object a zone's `controller` field references; same shared `sdn.apply`, no bespoke path), `sdn.ipam.create/update/delete` (T-3104 — the configured IPAM plugin *object* itself (netbox/phpipam/pve connection config), not an allocation, which a zone's `ipam` field references; same shared `sdn.apply`, no bespoke path), `sdn.apply` |
| guest | `guest.nic.update` (reattach bridge/vnet, vid, rate, firewall flag) |
| fw | `fw.rule.create/update/delete/move`, `fw.options.update`, `fw.alias.*`, `fw.ipset.*`, `fw.group.*` — **T-1602's microsegmentation planner (`internal/microseg`) reuses this group unchanged**: a proposed policy stages as ordinary `fw.rule.create` ops (the ACCEPT allow-list plus one trailing match-all `DROP` per governed direction — the whole default-deny policy expressed as rule-creates, no new op type, no schema change, no second mutation path). The planner only PROPOSES and dry-runs (`POST /microseg/propose`/`dry-run`, docs/api.md); a human applies through the ordinary changeset lifecycle. **T-3103** adds `direction: "forward"` (cluster/node/vnet scope) and `fw.options.update`'s `defaultForward`/`logLevelForward` fields, plus a fourth target scope — a vnet's forward-chain ruleset, `Ref{Kind: "fw-ruleset", ID: "vnet/<zone>/<vnet>"}` (mirrors `guest/<kind>/<vmid>`'s "kind/... composite in ID" convention). No `fw.alias.*`/`fw.ipset.*`/`fw.group.*` op may target vnet scope — real PVE exposes no such endpoint there (hardware-captured). |
...
| tc.mirror (T-4014) | `tc.mirror.create/update/delete` — a node-local `tc`/clsact/mirred SPAN/mirror session (params: `sourceIface`, `destIface`, `maxDurationSec` (required, positive — a mirror session must have a bounded, self-stopping maximum duration), optional `maxMbit` (a declared, validate-time-only bandwidth ceiling — never a kernel `police` action, see `internal/tcmirror`'s doc comment)); target is a `tc-mirror` Ref (node-scoped, caller-chosen id, no dedicated live-polled inventory kind of its own — like `qos-shape`). `tc.mirror.update`'s only mutable field is `maxDurationSec` (re-arming the session's own expiry deadline) — `sourceIface`/`destIface` are immutable after create, like `nat.*`'s identity fields. **Deliberately diverges from packet capture's mechanism** (`internal/capture`, gated on `auth.CapCapture` alone, no changeset op) because a mirror session is a *standing* config change, not a bounded diagnostic run — but reuses capture's bounding+audit *discipline*: a hard, server-enforced maximum duration enforced by `internal/change/tcmirror_expiry.go`'s unattended sweep, which tears down an overdue session via an ordinary, system-drafted `tc.mirror.delete` changeset (never a second mutation path) — see that file's doc comment for the full mechanism and the daemon-restart guarantee. |
...
**T-4014's `tc.mirror.*` ops are node-local but NOT node-file ops**, the identical shape `qos.shape.*` establishes just above: a session's on-node realization is a `tc`/clsact qdisc + matchall filter + `mirred` action invocation (`internal/tcmirror.RenderTC`), executed by its own daemon-level `TcMirrorGateway` seam (`internal/change.TcMirrorGateway`, the same "no user PVE ticket needed" shape as `QosGateway`, so its rollback works on the unattended commit-confirm-timeout path too) and its own apply-step kind (`tc_mirror_apply`), ordered after every node's interface stage/reload pair (source/dest ifaces must already exist) and before a trailing `sdn.apply`. A session's intent + accounting is persisted in the app-owned `tc_mirror_sessions` table (§2 above) — the live tc/clsact state itself is never shadow-copied there. A `tc.mirror.create`/`update` whose `destIface` names a node's resolved management/corosync path is rejected outright by `validate_safety.go`'s `tcMirrorProtectedDestFindings` (reusing `protected.go`'s `ProtectedSet` directly, the identical interlock and `codeProtectedInterface` finding code every other mgmt-path-cutting op already carries) — this is a direct static check on the declared destination, not the broader `touchesMgmtPath`/T-703 confirm-window-floor ceremony `qos.shape.*`'s `bridge` field inherits, since a mirror session never modifies the management interface's own config, only asks whether it may be repurposed as a copy destination.

**`tc_mirror_sessions` (T-4014, migration `internal/store/migrations/0053_tc_mirror_sessions.sql`).** App-owned intent + accounting only, per this doc's top-level rule: `id` (the op target's own id), `node`, `source_iface`, `dest_iface`, `max_mbit` (nullable — a declared, validate-time-only bandwidth ceiling), `max_duration_sec`, `status` (`active`\|`expired`\|`stopped`), `created_by`, `started_at`, `expires_at` (`started_at + max_duration_sec`, the durable, daemon-restart-safe deadline `internal/change/tcmirror_expiry.go`'s sweep checks), `stopped_at` (nullable). Written by the `tc.mirror.*` apply/rollback executor (`cmd/vnproxd`'s `hostTcMirrorGateway`) or by the daemon's own unattended expiry sweep (which applies an ordinary `tc.mirror.delete` changeset, never a direct row/tc mutation outside the change engine) — there is no third mutation path.
