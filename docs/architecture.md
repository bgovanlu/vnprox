...
- **Peer host writes are validated and audited on the receiving side too (T-2902).** Authentication says who may call; it does not say the call is safe. A `/api/peer/host/{stage-interfaces,restore}` body runs the receiving node's own change-engine safety validation (the raw-content pipeline, protected-path interlocks included) before its host writer is touched — with restores of snapshot-known content exempted by provenance so distributed rollback keeps working — and every host write is appended to the receiving node's audit log with the coordinator-supplied attribution (`actor`/`originNode`/`originIP`, additive at protocol 2). The coordinator's validation still runs first; the receiving check is defense in depth, so both halves of a two-daemon write enforce the same interlocks.
