...
- **Validation** is layered: schema (types/ranges), referential (does `bond0` exist; is `eno1` already enslaved), and **safety** (refuses ops that would orphan the node's management IP or the vnprox/corosync links unless explicitly overridden). **Correction (flagged, T-607 docs audit):** a fifth "cross-node consistency" pre-apply validator class (e.g. "warn when a bridge exists on 2 of 3 nodes") is described here and in `docs/features/change-management.md` §2 but does not exist in `internal/change` — cross-node divergence is instead caught only after the fact by the async drift checker (`docs/features/topology.md` §6, 30s interval), not blocked/warned on at validate/apply time. See that doc's §2 for the fuller correction and follow-up note; this is a real, currently-accepted gap, not a wording nit.
