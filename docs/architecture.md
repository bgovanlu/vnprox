...
- **Validation** is layered: schema (types/ranges), referential (does `bond0` exist; is `eno1` already enslaved), **safety** (refuses ops that would orphan the node's management IP or the vnprox/corosync links unless explicitly overridden), and cross-node consistency (warn when a bridge exists on 2 of 3 nodes).
- **Apply** produces an explicit ordered plan (visible to the user before apply): per-node steps, PVE API steps, and an SDN apply step where needed. Node-network steps use PVE's `PUT /nodes/{node}/network` + reload (ifupdown2), which is itself transactional per node; vnprox adds the cluster-level ordering and the commit-confirm envelope.
