...
| T-4002 | **Ansible collection.** Modules mirroring the provider's stage-only contract, plus a dynamic inventory source built from the topology. **Liveness semantics are settled: follow [ADR-0012](../docs/adr/0012-stage-only-integrations-never-imply-liveness.md) — expose a refreshed `result.changeset.status`, never imply the network converged, do not wait for a human. Do not re-derive an answer.** | L | T-4001 patterns; ADR-0012 |
| T-4003 | **Runbooks.** Parameterized sequences of read-checks and changeset templates attached to finding types; "prepare remediation" stages and stops. The MCP/plugin write boundary (stage, never apply) already defines the contract. **[ADR-0012](../docs/adr/0012-stage-only-integrations-never-imply-liveness.md) binds here too: a prepared runbook reports *prepared*, never *remediated*, and publishes the same status-plus-gate pattern.** | L | ADR-0012 |
